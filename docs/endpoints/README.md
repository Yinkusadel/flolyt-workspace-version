...
| Inbox | [inbox.md](inbox.md) | 11/11 documented, 9/11 wired into `/inbox` and live-verified (2 — thread read/reply — wired but not live-exercised, no `Message`-kind item in the test account); `read-all` + 3 draft endpoints still service/hook-only |
