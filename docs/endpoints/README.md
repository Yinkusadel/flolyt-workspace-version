...
| Inbox | [inbox.md](inbox.md) | 11/11 documented, 9/11 wired into `/inbox`, all 9 live-verified (thread read/reply confirmed 2026-09-19 once a real `Message` item existed); `read-all` + 3 draft endpoints still service/hook-only; 2 open issues found live 2026-09-19, not yet fixed (see docs/inbox/build-plan.md) |
...
| Leakage | [leakage.md](leakage.md) | 9/9 documented and scaffolded (service+hook), 0/9 wired; supersedes the never-wired `ROOMS.OPEN_ROOM_ON_LEAKAGE_CELL` guess (see file); 2 responses truncated in the paste, flagged in the file |
