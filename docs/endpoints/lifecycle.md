...
- **Response `data`:** `{ stageKey, stageName, measurementAgeDays: number[], cohorts, undatedCustomers, valueCaveat, caveat, callouts: [...] }`. **Corrected 2026-09-05, confirmed live for `expand`:** `cohorts` is itself the measured-value wrapper (`{value, state, missingSource, wouldUnlock}`), same recurring shape as `GET /map`'s `atStake` and (as of the same day) `GET .../compare`'s fields — not a bare nullable array as previously typed. Expand's live response came back `state: "unavailable"` (`missingSource`: "a definition for Expand — nothing yet says who enters it"), which confirms the wrapper but **still does not confirm the per-cohort row shape inside `.value`** — no live response with actual rows has been seen yet. Don't invent that row shape; confirm against one live response before rendering a table. `measurementAgeDays: [180]` confirmed live for Expand, matching the prose's claim for expand/advocate/churn. **Note:** the previously-documented top-level `currency` field is gone from this pass — `values` (see Notes) is per-currency instead, one figure per currency and never summed, consistent with the rest of this domain.
- **Used by:** service + hook exist (`get-stage-cohorts.ts` / `use-get-stage-cohorts.ts`, wrapper corrected 2026-09-05), **not wired** — target `stage/{acquire,...}/cohorts-tab.tsx`, which today is 10 different bespoke per-stage designs (same situation Markets was in before its 2026-09-05 rewrite), none matching this endpoint's generic shape. Held deliberately per the user's explicit call: the per-cohort row shape inside `cohorts.value` is still unconfirmed by any live response, and Cohorts additionally needs a bigger redesign (replacing 10 bespoke components) than Markets did — wait for a live response from a stage with real cohort data (not another `unavailable` case) before building the table.
