...
- **Used by:** `get-leakage-map.ts` / `use-get-leakage-map.ts`, wired 2026-09-01 to `/rooms/new`'s Step 1 (condition step) as the `conditionKey` vocabulary picker — grouped in the dropdown by `grid` (deduped by `condition.key` across grids, since `POST /rooms/new` only takes a bare `conditionKey`, never `grid`). Still a candidate source for the 7 stages' `atStake` that `/map` itself can't provide (see coverage tracker, needs backend confirmation) — that use is separate and still not wired. `?stage=` filter itself not wired anywhere yet — target is each stage's own leak-table section (see per-stage screens, §01 of `lifecycle-reference.md`). **Second consumer, wired 2026-09-06:** `revenueModel` alone (ignoring `grids`) now also backs the app shell's sidebar badge — `app-layout.tsx` calls this same hook app-wide just to read that one field, mapped `consumer`→"Consumer"/`account_based`→"Accounts"/`both`→"Hybrid" in `sidebar.tsx`. This is currently the **only** endpoint in the app that surfaces the workspace's saved revenue model — `PUT /workspace/revenue-model` (see `workspace.md`) is write-only, and `GET /workspace/profile`'s superficially similar `businessModel` field is a distinct, AI-derived free-text field, not this closed vocabulary. Worth moving to a cheaper/more natural endpoint (`/workspace/profile` or `/workspace/onboarding`) if backend ever adds it there, rather than fetching a revenue-leakage grid app-wide just for one field.
- **Status:** partially wired (rooms/new condition step; `revenueModel` also wired app-wide for the sidebar badge, 2026-09-06).
...
- **Used by:** service + hook exist (`get-stage.ts` / `use-get-stage.ts`) — **wired 2026-09-05 for all 10 stages**, both the Overview tab's KPI numbers AND its leak table (`departures[]`) now — the bar-rows comparison (Acquire only) and every stage's insight-callout text remain `data.ts` mock, unrelated to this pass. Unlike Cohorts/Markets, this endpoint returns the **identical 4 generic KPI fields for every stage** — no per-stage shape mismatch to reconcile, only a labeling choice. `overview-tab.tsx` now has one `buildStageKpis(stageData, stageSlug)` function (replacing the earlier per-stage `buildAcquireKpis`/`buildActivateKpis` pair) with a small `POPULATION_LABEL`/`PRIMARY_CONVERSION_LABEL` lookup so each stage keeps its own established card wording (e.g. Retain's population card was already "Acquired · 12 months" in its own mock; Adopt's conversion card reuses "Adopting · 2+ features") while all 10 stages render the same 4 cards in the same order (population/atStake/rateOfChange/primaryConversion). Each stage's other bespoke 3rd/4th KPI ideas (Price's "Contribution margin", Expand's "ARPU multiple", Advocate's "Effective CAC", Retain's "Ageing out", etc.) have no backing field and are dropped, same treatment as Acquire's "Blended CAC"/Activate's "Median time to value". All 8 remaining stages' `*_OVERVIEW_KPIS` mocks removed (Support's was renamed to `SUPPORT_SILENT_FAILURES_KPIS` and kept — it's also reused by the unrelated `silent-failures-route.tsx` drilldown, which stays fully mock).

  **Corrected 2026-09-06, twice, both caught by the user.** First pass: the KPI-card labeling
  reused every stage's OLD MOCK card label assuming it still described what the live field
  measures — but this doc's own coverage-tracker note (2026-08-31, above) already said that
  mapping is unsafe for 8 of 10 stages: "Acquire/Activate's 'Acquired · 12 months' card is a real
  `population` match; Retain's identical card is NOT its own `population` — it's echoing Acquire's
  top-of-funnel number for context, a different concept than 'who's in Retain now'." That note
  went unread before the label lookup was built. First fix kept Acquire/Activate's specific labels
  (treating that note as confirming them) and genericized the other 8.
  **Second pass, on closer reading, corrected that too**: every "confirmed 2026-08-31" note
  anywhere in this doc is about the measured-value **wrapper shape**
  (`{value,state,missingSource,wouldUnlock}` vs. a bare number) — none of them confirm what time
  window `population` actually covers. "Acquired · 12 months" is Figma mock copy, never verified
  against a live response naming a 12-month period. The note itself ends "❓ open — this needs the
  same per-stage audit the metric line got before wiring anything," meaning even its own author
  left this unsettled for every stage, Acquire/Activate included. **Final state: no stage gets a
  specific KPI label** — `buildStageKpis` (no longer taking a `stageSlug` parameter, since there's
  nothing left to look up) renders the plain field name for every card, all 10 stages identically:
  "Population" / "At stake" / "Rate of change" / "Primary conversion". The two Acquire/Activate
  per-stage notes ("the number that decides if this is good", "activation, not just a first
  order") were dropped too, since they presupposed the now-removed specific label.
  Same overall fix also corrected 7 of 10 stages' leak-table `leakEyebrow` heading, which baked in
  the OLD MOCK's specific stale totals ("Where the 528,000 who never activate are lost", "Where
  the ₦124M is", etc.) — numbers with nothing to do with the live `departures[]` data now under
  them (which reads empty for every stage today). All 7 replaced with the generic "What is
  leaking, in order" heading Acquire/Price already used; Advocate's non-numeric heading was left
  as is. **`insightTitle`/`insightBody` (the narrative Callout) still carry the same class of
  stale specific-number mock text and have NOT been touched** — flagged to the user as the same
  problem, not yet fixed.

  **Leak table, 2026-09-05, all 10 stages**: replaced every stage's mock leak-row list with `departures[]`, grouped by exit-rule cause. Columns: Where←`cause`, Customers←`size`, Value←`observedValue` (formatted via `formatCompactCurrency`, no currency field on a departure — same assumption `atStake` already makes), Trend←`trend.direction` (+`shareChange` when present), Cause known?←`claim.grade` in a defensively-toned `Chip` (`claimTone()`, keyword-matched — grade's real enum values are unconfirmed), Room←`roomOpen` boolean in a clickable `Chip` that still opens the stage's one static `OpenRoomPreset` (unchanged pre-existing behavior — that modal was already using one preset per stage regardless of which row was clicked, not per-departure content). **The old per-stage "Owner" column (Acquire only) is dropped** — no owner field exists on a departure, so every stage now uses the same "Cause known?" column. `LeakRow`/`RoomBadge` types and every stage's `*_OVERVIEW_LEAK_ROWS` mock removed (11 files: acquire's type definition + all 10 stages' row arrays).

  **Important shape caveat**: `observedValue`/`reachability` on `StageDepartureDto` are typed as a plain `value + *Caveat` pair (e.g. `observedValue: number | null` + `observedValueCaveat: string | null`), **not** the 4-field `{value,state,missingSource,wouldUnlock}` wrapper used everywhere else in this domain — rendered accordingly (caveat string stands in for `missingSource`, no `wouldUnlock` shown). This has never been confirmed by a live response with actual departure rows (every real response so far returned `departures: []`), so the empty state is what every stage honestly shows today — not a bug, not incomplete loading.

  Verified two ways: network-mocking a fake authenticated session in Playwright with a crafted `GET /lifecycle/stages/price` KPI-only response (confirmed correct labels/values for a stage that had never been touched before), and a second crafted response carrying two synthetic `departures[]` rows (one fully available, one fully unavailable) plus the real empty-array case — all rendered correctly: composed trend text, `InfoTooltip` on nulls, clickable Room chip, and the honest "No departures measured yet" empty state. Not yet checked against any live response carrying real departure rows.
- **Status:** wired (all 10 stages), live-verified only for Acquire.
...
- **Response `data`:** `{ stageKey, stageName, measurementAgeDays: number[], cohorts, undatedCustomers, valueCaveat, caveat, callouts: [...] }`. **Corrected 2026-09-05, confirmed live for `expand` (unavailable), then `activate` and `retain` (real rows):** `cohorts` is the measured-value wrapper (`{value, state, missingSource, wouldUnlock}`), same recurring shape as `GET /map`'s `atStake` and `GET .../compare`'s fields — not a bare nullable array as previously typed. `.value` is `{periodStartUtc, entered: MeasuredValue<number>, values: MeasuredValue<{currency, amount, perCustomer}[]>, ages: [{ageDays, stillInStageShare: MeasuredValue<number>}]}[]` — confirmed field-for-field against two real responses. `stillInStageShare` read unavailable throughout both, exactly matching the prose: a lifecycle-bridged stage (no explicit definition) records where a customer is *now*, not where they stood at 30/60/90 days. `entered`/`values.amount` of `0` is a genuine "available" measurement (the pass ran and found nobody that month), not a missing one. `perCustomer` is provided by the API (`amount ÷ entered`), never computed client-side. `measurementAgeDays: [180]` confirmed live for Expand, `[30,60,90]` for Activate/Retain, matching the prose. **Note:** the previously-documented top-level `currency` field is gone from this pass — `values` is per-currency instead, one figure per currency and never summed, consistent with the rest of this domain.
- **Used by:** service + hook exist (`get-stage-cohorts.ts` / `use-get-stage-cohorts.ts`, full row shape typed 2026-09-05) — **wired 2026-09-05**, `stage/cohorts/cohorts-tab.tsx`. **Same situation as Markets before its rewrite**: cohorts wasn't a real shared template — 9 of 10 stages had their own bespoke per-stage table (Acquire's CAC/day30-90, Activate's activation-rate, Retain's repeat-rate, etc.), none matching this endpoint's one generic age-aligned shape. All 9 bespoke `*/cohorts-tab.tsx` files deleted, replaced by one generic table (Cohort/Entered/one column per `measurementAgeDays` entry/Value to date) used by all 10 stages. Advocate's and Churn's "Open a war room" header button (same pattern found on their old Markets tabs) dropped for the same reason — seeded with a hardcoded finding tied to the fabricated mock, no grounding once the data is real. `valueCaveat`/`caveat` render as their own callouts when present; `undatedCustomers > 0` renders a small note. **Verified against both live responses by network-mocking a fake authenticated session in Playwright** (cookie-based auth — `auth_token`/`user_data`/`refresh_token` — plus intercepting the API host to fulfill the cohorts endpoint with the real captured JSON and everything else with an empty 200) rather than blind smoke-testing; screenshots confirmed correct rendering for both the real-rows case (Retain) and the unavailable case (Expand). **Also fixed the same day**: `formatCompactMoney` had no billion-scale breakpoint — Retain's real `amount: 2178452653.5` would have rendered as "₦2178M" instead of "₦2.18B" until a `>= 1_000_000_000` branch was added.
- **Status:** wired (all 10 stages), live-verified for `expand` (unavailable), `activate` and `retain` (real rows).
- **Notes:** `measurementAgeDays` is `[30, 60, 90]` for most stages, `[180]`-inclusive for expand/advocate/churn — confirmed live for Expand/Activate/Retain. Each cohort row carries one cell per age with `stillInStageShare` (0-1): a cell is available once the cohort's **last** arrival has lived the whole window, else unavailable naming the date it matures — measured then, never projected/interpolated. `entered` (arrivals count) is itself a measured value: an available zero means the pass ran and genuinely found nobody, while a month newer than the matrix's own compute date reads unavailable naming that date — an uncounted month must not read the same as an empty one. A **defined** stage cohorts from its own warehouse (six-hourly refresh); a lifecycle-**bridged** stage without a definition carries `entered`+observed value with every age cell unavailable, because the bridge's stand-in records where a customer is *now*, not where they stood at day 30. `values` (observed money to date, **never at-stake, never a forecast**) is lifetime revenue in billing currency on bridged stages, or the entry event's per-currency amounts on defined stages — one entry per currency, never summed, unavailable naming the missing column/currency when nothing could be totalled. `undatedCustomers` (bridged stages only) counts members whose entry was never dated — excluded from the matrix itself, with `caveat` saying so. `callouts` includes a dated-break reading when adjacent same-age cohorts fall sharply enough to name.
...
- **Used by:** service + hook exist (`get-stage-markets.ts` / `use-get-stage-markets.ts`) — **wired 2026-09-05**, `stage/markets/markets-tab.tsx`. **This replaces a much bigger mismatch than History/Agents/Compare did**: Markets was never actually a single shared template before this — 9 of 10 stages each had their own bespoke component (Activate's activation-rate/guest-share, Price's FX-drift/repricing, Retain's repeat-rate/reactivable-now, etc., only Acquire used a generic spend/CAC/reach-2nd-order fallback), none of which this endpoint's 3-field shape (population/atStake/primaryConversion per market) can back. All 9 bespoke `*/markets-tab.tsx` files deleted, replaced by one generic table (Market/Population/At stake/Primary conversion) used by all 10 stages — same dispatch-free pattern as `AgentsTab`/`HistoryTab`/`ChangesTab`. Country names shown via a small static ISO-code lookup (a presentational label, not a fabricated field) with the raw code as fallback. Advocate's and Churn's "Open a war room" header button (the only two stages with any header action on this tab) is dropped too — both were seeded with a hardcoded Ghana/Kenya-specific finding tied to the old fabricated narrative, with no grounding once the table is generic real data. Churn's bespoke "Ghana across all ten stages" second table is dropped the same way. All 9 stages' `*_MARKET_ROWS`/`*_MARKET_CLOSING`/`*_MARKET_GHANA_ROWS`/etc. mocks removed, plus Acquire's own `MarketRow`/`ACQUIRE_MARKET_ROWS`/`ACQUIRE_MARKET_SPOTLIGHTS`. Not yet checked against a real response.
- **Status:** wired (all 10 stages, single generic component), not yet live-verified.
...
- **Response `data`:** `{ stageKey, stageName, windowMonths, before: {fromUtc, toUtc, monthsInWindow, monthsMeasured, endPopulation, averagePopulation, endConversion, restatedMonths}, after: {...}, change, changePercent, conversionChange, definitionChangedInside, callouts: [...] }`. **Corrected 2026-09-05, confirmed live for `adopt`:** `endPopulation`/`averagePopulation`/`endConversion`/`change`/`changePercent`/`conversionChange` are each the measured-value wrapper (`{value, state, missingSource, wouldUnlock}`), same as `GET /map`'s `atStake` — not the bare `number | null` the written spec's example implied (same recurring lesson as `get-stage.ts`'s `population`/`rateOfChange`/`primaryConversion`, now confirmed for this endpoint too). `fromUtc`/`toUtc`/`monthsInWindow`/`monthsMeasured`/`restatedMonths` are plain, unwrapped.
- **Used by:** service + hook exist (`get-stage-compare.ts` / `use-get-stage-compare.ts`) — **wired 2026-09-05**, `stage/compare/compare-route.tsx` (shared across all 10 stages). Two rows only: Population (`endPopulation`, change preferring `changePercent` and falling back to the raw `change` count — never formatting a raw count as a percent) and Primary conversion (`endConversion`/`conversionChange`). **"What moved it" and the fuller CAC/repeat-rate/value-per-customer table are dropped** — this endpoint never attributes a cause and only ever compares population+conversion, so those columns/rows have no field to read (Acquire's "how this comparison is built" static notes dropped too, same reason). The period-selector buttons now call the real `months` query param (3/6/12 presets) instead of the mock's "Year on year"/"Custom"/market-split options, none of which this endpoint actually supports (no date-range or market-filter param). `definitionChangedInside` renders as its own callout. All 10 stages' `*_COMPARE_ROWS`/`*_COMPARE_BUILD_ROWS`/`*_COMPARE_INSIGHT` mocks removed. **First-pass bug, caught live and fixed same day**: the service file originally typed every one of the fields above as a bare `number | null` (per the written spec's example) — rendered as literal `[object Object]` for values and `NaN%` for every Change cell the moment a real response came back. Fixed by rewrapping all six fields in `LifecycleMeasuredValueDto<number>` and unwrapping `.value`/`.missingSource`/`.wouldUnlock` through the same `InfoTooltip` pattern every other wired tab uses.
- **Status:** wired (all 10 stages), live-verified for `adopt` (all fields returned unavailable — a fresh workspace with no measured months in any window).
...
- **Status:** **✅ wired 2026-09-05** — `stage/adopt/features-tab.tsx`. No field for orders/month-after, ship date, or a verdict per feature, so those and the old mock's per-feature drilldown link are dropped. **Correction, live-confirmed 2026-09-05:** `kept`, `abandoned`, and top-level `customersSeen` are all the measured-value wrapper, not bare `number | null` — pre-fix UI showed `NaN%`/`[object Object]`. Also confirmed live: `callouts[].tone` includes `"attention"` and `"insight"`, outside the originally-assumed amber/teal/rose/ultra/neutral set — matched defensively by keyword now, same pattern as `claimTone`.
...
- **Status:** **✅ wired 2026-09-05** — `stage/adopt/depth-tab.tsx`. No first/second-feature pairing field, so the old mock's "which second feature matters most" table is dropped. **Correction, live-confirmed 2026-09-05:** `stillActiveShare`, `medianFeatures` and `lift` are all the measured-value wrapper, not bare `number | null` — pre-fix UI showed `NaN%`/`[object Object]`. `callouts[].tone` also confirmed to include `"context"`, matched defensively same as Features' `"attention"`/`"insight"`.
...
- **Status:** **✅ wired 2026-09-05, measured-value + date bugs fixed 2026-09-06** — `stage/expand/basket-tab.tsx`. Same surprise as Price's Margin tab: `months[]`/`movement[]` return real data independently of `order_lines`; only `lines[]` (item composition) needs it and is shown only when present, with a callout otherwise. The old mock's specific "before/after the fee" narrative numbers aren't reproducible either way. **Correction 2026-09-06, caught by the user from a live response:** `averageOrderValue`/`ordersPerCustomer`/`revenuePerCustomer` (per month) and `revenuePerCustomerChange`/`averageOrderValueChange`/`ordersPerCustomerChange` (per movement row) were typed as bare `number | null` but the live response returns the same measured-value wrapper (`{value, state, missingSource?, wouldUnlock?}`) used everywhere else in this domain — the same recurring bug class as `backtest`'s `firings` field. Rendering an object where a number was expected produced literal `"₦[object Object]"` in the Avg order value / Revenue per customer cells and `NaN` in Orders/customer. Fixed in `get-expand-basket.ts` (both DTOs now type these fields `LifecycleMeasuredValueDto<number>`) and `basket-tab.tsx` (reads `.value`, shows `InfoTooltip` when unavailable). **Second bug found fixing the first:** `revenuePerCustomerChange` is a *fraction* (e.g. `-0.0028`), not a currency delta — the movement row was formatting it with `formatCompactMoney` regardless; switched to `formatPercent`. **Third, unrelated bug, same session:** `period`/`from`/`to` are raw un-ISO datetime strings (`"01/01/2026 00:00:00"`, MM/DD/YYYY) rendered verbatim — now passed through `formatMonthYear` (`new Date(...)` parses this slash format correctly in this app's Chromium/V8 runtime) for a "Jan 2026" style label instead. **Also removed 2026-09-06, unrelated to the response shape:** the header's static "Request instrumentation" button (`EXPAND_REQUEST_INSTRUMENTATION_PRESET`, deleted from `stage/expand/data.ts`) — it hardcoded the exact `order_lines` gap this tab's own "Basket composition is unavailable" Callout already names, with a frozen "41 days overdue" that could never update and a "Send to Engineering" that never actually sent anything. Now that `order_lines` (if it's a real `gapKey`) is requestable for real from the workspace-wide gaps section on `/lifecycle` (see `POST /instrumentation-requests`'s entry below), keeping a second, fake, per-tab button for the same ask was actively misleading. `stage/modals/request-instrumentation-modal.tsx` (the shared static-preset component) is deleted too — this was its last consumer after Adopt's Blind Spots tab was removed the same day.
...
- **Status:** **✅ wired 2026-09-05** — `stage/acquire/unit-economics-tab.tsx` via `use-get-acquire-unit-economics.ts` / `get-acquire-unit-economics.ts` (both already scaffolded from an earlier pass; this doc's "not scaffolded" line was stale). The old mock was a per-channel CAC/revenue-per-customer/ratio table — this endpoint has no channel dimension at all, only cohorts, so that table was dropped rather than reshoehorned onto the wrong grouping. What's wired: a per-cohort row (customers, months observed, `acquisitionCost`), `revenuePaybackMonth`/`marginPaybackMonth` rendered as "Month N" or "Not yet crossed" (never "Unavailable" — per this endpoint's own "not-yet is not never" rule, these are plain nullable numbers, not the measured-value wrapper), and a small inline SVG curve per cohort (`points[].cumulativeRevenue`/`cumulativeMargin`, no charting library added) with a dashed reference line at `acquisitionCost` — built to honor this endpoint's own "the shape is the point" emphasis rather than collapsing to one number. The "Margin payback" column and margin curve line only render when `hasMargin` is true; `costComponents` surfaced as a plain sentence when present. The old "Connect a COGS source" CTA (`MapAFieldModal`) kept as-is, gated on `!hasMargin`. Live-verified only via network-mocked Playwright (hand-built payloads matching this DTO shape) across both `hasMargin` true/false — not yet checked against a real backend response.
...
- **Response `data`:** `{ stageKey, stageName, metricKey, metricQuestion, threshold, sustainReadings, firings, grain, points: [{periodStartUtc, reading, breaching, wouldHaveFired}], caveat }`. **`firings` is the measured-value wrapper** (`{value, state, missingSource, wouldUnlock}`), confirmed live 2026-09-06 on Expand/`population` (never counted → `state: "unavailable"`) — corrected from an initial `number | null` type that crashed rendering it directly (React/Preact: "Objects are not valid as a child"), the same recurring bug class this whole domain has hit repeatedly; see the standing lesson in [[flolyt_lifecycle_endpoints]].
- **Status:** **✅ wired 2026-09-05, `firings` typing fixed 2026-09-06, stale-result bug fixed same day** — `stage/modals/backtest-preview.tsx`'s `BacktestPreview`, shared by the create, edit and accept-with-changes modals as a "Simulate" button + live preview against the in-progress draft values (replacing the old static per-stage `simulation` mock text). Renders `firings.value`/`grain`/`caveat` as text (via `InfoTooltip` when `firings.state === "unavailable"`) and `points[].reading` as a `Sparkline` (see `stage/sparkline.tsx`) with `threshold` drawn as a dashed reference line. Disabled when the selected metric's `hasHistory` is `false` (from `GET /watchable-metrics`), with the reason shown inline instead of letting a doomed request fire. **Caught by the user 2026-09-06**: a prior simulation's result stayed on screen after editing the metric/threshold/sustain/segment it was simulated against, silently going stale — fixed with a `useEffect` that calls the mutation's own `reset()` whenever any of those inputs change, clearing the result immediately so a mismatched preview is never shown.
...
- **Status:** **✅ wired 2026-09-06, two real-submission bugs found and fixed same day** — `RequestGapInstrumentationModal` (`pages/everyday/lifecycle/modals/request-gap-instrumentation-modal.tsx`), opened per-row from the new gaps section on `/lifecycle` (see `GET /lifecycle/instrumentation`'s entry above). `gap`/`gapKey`/`blocks` are carried straight through from the gap row. **The old `stage/modals/request-instrumentation-modal.tsx` and both its consumers are now deleted** — Adopt's Blind Spots tab (removed the same day this endpoint was wired) and Expand's Basket tab's own static "Request instrumentation" button (removed shortly after, once the user pointed out it was hardcoding the exact same `order_lines` gap this endpoint now serves for real — see that tab's own entry above). Neither preset's "Send to Engineering" had ever called this endpoint; both just closed the dialog and toasted.
  - **Bug 1, caught by the user's first real submission:** `requiredEventSchemas` was assumed pre-populated from the gap row (`gap.requiredEventSchemas`) and sent `null` when that array was empty — the backend flatly refuses this: *"Name the events being asked for. Without them this is a conversation rather than a contract."* Contrary to the spec's `?` marking it optional, **this field is required in practice, at least one non-empty entry, on every submission** — not every gap's `requiredEventSchemas` comes pre-filled by the backend. Fixed by making it a real editable field: a dynamic add/remove list of text inputs, seeded from the gap's own list when one exists (Adopt's `loyalty-tiers` gap, say) but left as one blank input to fill in by hand when it doesn't. Confirm is disabled until at least one trimmed, non-empty schema name exists.
  - **Bug 2, caught by the user's second real submission:** `neededByUtc` was built as `` `${date}T00:00:00Z` `` — the *start* of the picked day. Picking today (allowed by the date input's `min`) produced a timestamp already in the past by the time the request reached the server, refused: *"A date already past is not a deadline. Give it a date somebody can still hit."* Fixed by building the timestamp as `` `${date}T23:59:59Z` `` instead — the *end* of the picked day, matching how "needed by [date]" actually reads and making today a valid choice for the rest of the day.
  - **Both were only caught because the user submitted for real** — a network-mocked Playwright pass (still run, still useful for catching rendering/wiring regressions) can't catch a live backend validation rule it doesn't know about. See [[feedback_mutation_flows_need_live_submit]].
