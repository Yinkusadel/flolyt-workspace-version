...
- **Used by:** service + hook exist (`get-stage.ts` / `use-get-stage.ts`) — **wired 2026-09-05 for all 10 stages**, both the Overview tab's KPI numbers AND its leak table (`departures[]`) now — the bar-rows comparison (Acquire only) and every stage's insight-callout text remain `data.ts` mock, unrelated to this pass. Unlike Cohorts/Markets, this endpoint returns the **identical 4 generic KPI fields for every stage** — no per-stage shape mismatch to reconcile, only a labeling choice. `overview-tab.tsx` now has one `buildStageKpis(stageData, stageSlug)` function (replacing the earlier per-stage `buildAcquireKpis`/`buildActivateKpis` pair) with a small `POPULATION_LABEL`/`PRIMARY_CONVERSION_LABEL` lookup so each stage keeps its own established card wording (e.g. Retain's population card was already "Acquired · 12 months" in its own mock; Adopt's conversion card reuses "Adopting · 2+ features") while all 10 stages render the same 4 cards in the same order (population/atStake/rateOfChange/primaryConversion). Each stage's other bespoke 3rd/4th KPI ideas (Price's "Contribution margin", Expand's "ARPU multiple", Advocate's "Effective CAC", Retain's "Ageing out", etc.) have no backing field and are dropped, same treatment as Acquire's "Blended CAC"/Activate's "Median time to value". All 8 remaining stages' `*_OVERVIEW_KPIS` mocks removed (Support's was renamed to `SUPPORT_SILENT_FAILURES_KPIS` and kept — it's also reused by the unrelated `silent-failures-route.tsx` drilldown, which stays fully mock).
  ...
  **Corrected 2026-09-06, caught by the user**: the KPI-card labeling above initially reused every
  stage's OLD MOCK card label assuming it still described what the live field measures — but this
  very doc entry's own coverage-tracker note (written 2026-08-31, above) already said that mapping
  is unsafe: "Acquire/Activate's 'Acquired · 12 months' card is a real `population` match; Retain's
  identical card is NOT its own `population` — it's echoing Acquire's top-of-funnel number for
  context, a different concept than 'who's in Retain now'." That note went unread before the label
  lookup was built. Fixed: `POPULATION_LABEL`/`PRIMARY_CONVERSION_LABEL` now only cover Acquire and
  Activate (the two stages this doc actually confirms); every other stage shows the plain field
  name ("Population" / "Primary conversion") instead of an invented-sounding per-stage claim
  ("Adopting · 2+ features", "Ever won back", etc.) that was never checked against a live response.
  Same commit also fixed 7 of 10 stages' leak-table `leakEyebrow` heading, which baked in the OLD
  MOCK's specific stale totals ("Where the 528,000 who never activate are lost", "Where the ₦124M
  is", etc.) — numbers with nothing to do with the live `departures[]` data now under them (which
  reads empty for every stage today). All 7 replaced with the same generic "What is leaking, in
  order" heading Acquire/Price already used; Advocate's non-numeric heading was left as is.
  **`insightTitle`/`insightBody` (the narrative Callout) still carry the same class of stale
  specific-number mock text and have NOT been touched** — flagged to the user as the same problem,
  not yet fixed.
