...
- **Used by:** service + hook exist (`get-stage.ts` / `use-get-stage.ts`) — **wired 2026-09-05 for all 10 stages**, Overview tab's KPI numbers only, NOT its bespoke per-row leak-table narrative (see coverage tracker note on Overview) — the leak table, bar-rows comparison, and per-stage insight text all remain `data.ts` mock, unrelated to this pass. Unlike Cohorts/Markets, this endpoint returns the **identical 4 generic fields for every stage** — no per-stage shape mismatch to reconcile, only a labeling choice. `overview-tab.tsx` now has one `buildStageKpis(stageData, stageSlug)` function (replacing the earlier per-stage `buildAcquireKpis`/`buildActivateKpis` pair) with a small `POPULATION_LABEL`/`PRIMARY_CONVERSION_LABEL` lookup so each stage keeps its own established card wording (e.g. Retain's population card was already "Acquired · 12 months" in its own mock; Adopt's conversion card reuses "Adopting · 2+ features") while all 10 stages render the same 4 cards in the same order (population/atStake/rateOfChange/primaryConversion). Each stage's other bespoke 3rd/4th KPI ideas (Price's "Contribution margin", Expand's "ARPU multiple", Advocate's "Effective CAC", Retain's "Ageing out", etc.) have no backing field and are dropped, same treatment as Acquire's "Blended CAC"/Activate's "Median time to value". All 8 remaining stages' `*_OVERVIEW_KPIS` mocks removed (Support's was renamed to `SUPPORT_SILENT_FAILURES_KPIS` and kept — it's also reused by the unrelated `silent-failures-route.tsx` drilldown, which stays fully mock). Verified by network-mocking a fake authenticated session in Playwright with a crafted `GET /lifecycle/stages/price` response — confirmed correct labels, values, and the unavailable/`InfoTooltip` path all render right for a stage that had never been touched before. Not yet checked against a real response for the 8 newly-wired stages (Acquire was live-verified 2026-08-31, Activate was not).
