...
- **Used by:** service + hook exist (`get-stage.ts` / `use-get-stage.ts`), not wired — target Overview tab's KPI numbers, but NOT its bespoke per-row leak-table narrative (see coverage tracker note on Overview). Service file needs updating for the new fields above before this is trustworthy to wire.
- **Status:** service/hook ready (stale — missing 2026-09-04's new fields), not wired.
...
- **Used by:** not wired — no service/hook file exists yet (new endpoint as of the 2026-09-04 spec paste). **Target: the stage History tab** (`/lifecycle/{stage}/history`) — confirmed match, see the coverage tracker's "History tab" entry above.
- **Status:** documented, not scaffolded.
...
- **Purpose:** Churn's Chain tab — one dated cause traced across all 10 stages, the product's core thesis made checkable.
