...
/onboarding/team              (step 5, no Figma source — create-team page; /onboarding/team/:teamId
                                is the invite/roster page a "Invite team member" button hands off to)
/onboarding/finishing-up      (no Figma source, not a stepper step — purely cosmetic closing
                                screen between step 5's Continue and landing on "/")
```
...
| 5 — Your team (no Figma source) | `POST /teams`, `GET /teams`, `GET /teams/{teamId}`, `POST /{teamId}/invitations`, `POST /{teamId}/invitations/resend`, `DELETE /invitations/{invitationId}` (see `docs/endpoints/teams.md`, all 13 endpoints wired 2026-08-28) | [x] both pages built 2026-08-28, **not yet live-verified** | Built step by step, per the user's own explicit instructions each time (no Figma source works for this shape). **Piece 1 — `/onboarding/team`, `src/pages/onboarding/team/index.tsx`:** header + "Create team" button (always visible, top — a workspace isn't capped at one team, so it stays even once teams exist) + a right-rail "what creating a team does" aside matching the data step's `WhatSourceUnlocks`/`WhatYouCanAskNow` pattern, a `CreateTeamModal` (name + optional description, `useCreateTeam`), and a card per existing team (name/description/created date + "Invite team member" button). **Piece 2 — `/onboarding/team/:teamId`, `team-detail-route.tsx`:** a "Back to teams" link back to `/onboarding/team`, an "Invite member" button opening `InviteMemberModal` (email + role checkboxes off `USER_ROLES`, via `useInviteTeamMember`), a Members table (name/email/role chips/joined date/active-status chip) and an Invitations table (email/role chips/status chip/invited/expires/actions) off `GET /teams/{teamId}` (members+invitations in one call, no separate paginated invitations query used). Resend/Revoke buttons render only for a `status.toLowerCase() === "pending"` invitation — accepted/expired/revoked ones show no actions. **Updated 2026-08-28 (later):** `inviteTeamMemberSchema` (`src/validators/teams.ts`) gained a work-email `.refine()` — `PERSONAL_EMAIL_DOMAINS`, a best-effort mirror of common consumer providers (Gmail, Yahoo, Outlook, iCloud, etc.), not the backend's actual disallow list, which isn't published — so the invite form now rejects an obvious personal address inline before the request fires. `resendTeamInvitationSchema` shares the same schema but resend never runs it (the hook mutates directly with the invitation's own already-accepted email, no form/resolver involved), so this only affects the invite modal. `OnboardingAgentsRoute`'s `goToNextStep` targets `/onboarding/team` (changed from `/` the same day step 5's first piece shipped). **Updated 2026-08-28 (later still):** the "Back to teams" link moved to the bottom of `team-detail-route.tsx`, styled as the canonical primary-CTA button (see "Cross-cutting: primary CTA button convention" above) instead of a small top-of-page arrow link, per the user's direct request. **Also fixed the same pass — a real bug the user caught live:** `useResendTeamInvitation`/`useRevokeTeamInvitation`'s `isPending` is one shared boolean per hook instance, so with a single hook instance driving every row in the Invitations table, clicking Resend or Revoke on any one row disabled the Resend/Revoke buttons on *every* row, not just that one. Fixed by having both hooks also return `variables` (the mutation's own last-called arguments, from React Query) — the table now computes `isThisResending`/`isThisRevoking` per row by matching `resendVariables?.email`/`revokeVariables` against that row's invitation, so only the row actually being acted on shows a disabled/"...ing" state. `tsc --noEmit` and `vite build` both clean; signed-out Playwright passes against both `localhost:3000/onboarding/team` and `.../team/:teamId` confirmed correct redirects to `/auth/sign-in` with zero console errors — the actual create/list/invite/resend/revoke flow needs a real session, which this sandbox can't reach against the live backend host (see [[flolyt_onboarding_build]] lesson 9). **Updated 2026-08-28 (yet later):** three more fixes from the user's live testing. (1) `/onboarding/team`'s header (title + Create team button) is now `shrink-0` with only the team-card list itself scrolling (`md:min-h-0 md:flex-1 md:overflow-y-auto`, same bounded pattern the agents step's card grid already used) — previously the whole column scrolled together and a long team list could push the page past the viewport. (2) Each `TeamCard` gained a delete (`Trash2`) icon button opening a new shared `ConfirmModal` (`src/pages/onboarding/team/confirm-modal.tsx`) before calling `useDeactivateTeam` (`DELETE /teams/{teamId}`, a soft deactivate per the endpoint doc, not a hard delete). (3) The Invitations table's Revoke button now opens the same `ConfirmModal` instead of calling `useRevokeTeamInvitation` immediately on click — it was firing with no confirmation at all. `ConfirmModal` is generic (title/description/confirmLabel/pendingLabel/isPending/onConfirm) and shared by both call sites. **Updated 2026-08-28 (latest) — root cause of a live bug the user hit:** inviting with Administrator among the selected roles came back `{ succeeded: false, messages: ["Confirm this change with the code we emailed you."] }`. Per `flolyt-extras/auth-frontend-handoff.md`'s step-up table, `POST /teams/{teamId}/invitations` is conditionally gated by the `change_administrators` action — required only when Administrator is granted, exactly as hit. Fixed per the doc's own suggested UX ("attempt without a challenge id first..."): added `isStepUpRequiredMessage()` next to `useStepUpConfirmation` (`src/features/auth/use-step-up-confirmation.ts`) matching the exact backend phrase, and an `onStepUpRequired` option on `useInviteTeamMember` that fires it instead of the generic error toast. `invite-member-modal.tsx` now wires `useStepUpConfirmation({action: "change_administrators", onConfirmed: (challengeId) => { form.setValue("stepUpChallengeId", challengeId); form.handleSubmit(onSubmit)(); }})` and stacks `StepUpConfirmModal` on top of the invite dialog. **Not wired for resend** — the handoff doc's `change_administrators` row doesn't list `POST /{teamId}/invitations/resend`, only invite, member-roles-PUT and member-DELETE; untested whether resend is actually gated too, see [[flolyt_governance_stepup_reminder]]. **Correction 2026-08-29:** the step-up check only worked when the rejection arrived as a 200-with-`succeeded:false` — the user retested and still hit the plain error, because the backend actually rejects this case as a non-2xx (axios throws, landing in `onError`, which had no such check). Fixed by checking `isStepUpRequiredMessage` in both `onSuccess` and `onError`, and loosened the match from an exact string to a case-insensitive substring (`"confirm this change with the code"`) since the exact phrase was only ever confirmed from one live response. **2026-08-29 — Continue button added:** team creation is optional, so `/onboarding/team` gained a bottom "Continue" button (canonical CTA class) that works with zero or several teams, fires `POST /onboarding/progress` with `kind: "Finished", step: "team"` (fail-open — navigates on `onSuccess` and `onError` alike, same pattern every other step's Continue uses), then sends the user to a brand-new closing screen, `/onboarding/finishing-up` (`src/pages/onboarding/finishing-up/index.tsx`, no Figma source, not a stepper step). That page does no data work at all — a spinning ring, four rotating status lines and a linear progress bar over ~4.6s, then a checkmark payoff for ~0.9s, then `navigate("/")` automatically. Purely cosmetic per the user's explicit request ("lovely animations... just a frontend thing to make it seem like something is happening"). Route added as a plain leaf under the `onboarding` route tree — exempt from `ProtectedRoute`'s finished-onboarding guard like every other `/onboarding/*` route, so it renders regardless of what the backend's `GET /workspace/onboarding` says; landing on `/` afterward is where that guard re-asserts itself normally (bounces back to the correct incomplete step if the `Finished` progress save didn't actually take). **Corrected 2026-08-29, same day, per direct user feedback:** the Continue handler originally fail-opened — navigated to `/onboarding/finishing-up` on both `onSuccess` and `onError` of the progress save, matching every other onboarding step's Continue. The user pointed out this meant the button could "just go to next" without the `Finished` flag actually having been set. Changed to fail-closed for this one button only: it now reads `data.succeeded` from the `onSuccess` callback and only navigates when it's `true`; a `false`/error response shows an error toast and leaves the user on `/onboarding/team` to retry, rather than silently proceeding into a workspace that never got marked finished. **Confirmed live 2026-08-29** — `POST /onboarding/progress {kind:"Finished", step:"team"}` returned `{"data":"workspace","messages":["Setup marked finished. Anything still outstanding stays visible."],"succeeded":true}`, and the following `GET /onboarding` showed `finished: true` **even though** its own `steps` array still listed `{"step":"team","isComplete":false,"outstanding":["Invite someone"]}` — the user flagged this as looking broken, but it's the endpoint doc's own documented behavior ("Finishing does not hide what's still outstanding — don't treat finished:true as nothing left to show"): the `team` step's `isComplete` is derived from real state (has anyone actually been invited) independently of the top-level `finished` flag the `Finished` progress event sets. `ProtectedRoute` only reads `onboarding.finished`, never the per-step array, so this doesn't block anyone from entering the app — confirmed correct, not a bug. Also confirmed live: `resumeAt`/`steps[].step` use the plain key `"team"`, not `"your_team"` — dropped that now-known-unused speculative key from `protected-route.tsx`'s `RESUME_STEP_ROUTES`. |
