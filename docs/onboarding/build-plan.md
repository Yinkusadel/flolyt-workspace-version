...
| 3 — Your data (05/06) | `GET /datasources`, `GET /datasources/connected`, `POST /datasources/test-connection`, `POST /datasources/connect`, `POST /datasources/{id}/disconnect`, `POST /datasources/{id}/reconnect` (see `datasources.md`) | [x] built, **not yet verified live** | `/onboarding/data`, `src/pages/onboarding/data/`. Confirmed: the connector list belongs to the already-wired Datasource Management domain, not a new onboarding-specific endpoint. Sub-states not sub-routes, matching the pattern above: `sources` (05 — searchable card grid off the real 49-item catalog, a "Popular" row for Snowflake/PostgreSQL/Stripe/Shopify per the SVG, and a desktop-only "what one source unlocks" rail whose four dots light up per the categories among the user's active connections) and `connected` (a card list with reconnect/disconnect, disconnect using the required two-step typed-confirm + keep/delete-customers flow). Connect flow is the old dashboard's two-step details→credentials `ConnectSourcesModal` ported onto this app's `Dialog` and hooks, Connect gated on a successful Test. Vendor logos render via `cdn.simpleicons.org/{slug}` with an initial-letter fallback, same as the old dashboard. Clicking a connected source opens `ConnectionDetailPlaceholder` — screen 06's mapping table still has no real endpoint (its shape doesn't match `GET /mapping-quality`'s documented one), so that space is reserved rather than built against a guess. No step-up gating — `connect`/`disconnect` aren't in the gated-actions list. `tsc --noEmit` is clean; not yet exercised against a real API from the browser. |
