...
| 5 — Your team (no Figma source) | `POST /teams`, `GET /teams` (see `docs/endpoints/teams.md`, all 13 endpoints wired 2026-08-28) | [x] page built 2026-08-28 (create-team only), **not yet live-verified** | `/onboarding/team`, `src/pages/onboarding/team/`. Built strictly to the user's own spec (no Figma source for this shape): header + "Create team" button (only shown while no team exists) + a right-rail "what creating a team does" aside matching the data step's `WhatSourceUnlocks`/`WhatYouCanAskNow` pattern, a `CreateTeamModal` (name + optional description, `useCreateTeam`), and — once a team exists — a card showing its name/description/created date with an "Invite team member" button. That button navigates to `/onboarding/team/:teamId`, the same handoff shape as the data step's "Show mapping" but across a route instead of a sub-state (a team has a real id to key off of) — **that detail/invite/roster page is a separate, later piece and doesn't exist yet**, so the button's target 404s/blanks until it ships, same accepted pattern as step 3→4 and step 4→5 had before their targets existed. **Updated 2026-08-28 (later):** `OnboardingAgentsRoute`'s `goToNextStep` now targets `/onboarding/team` (was `/`) — see step 4's row above. `tsc --noEmit` and `vite build` both clean; a signed-out Playwright pass against `localhost:3000/onboarding/team` confirmed the route correctly redirects to `/auth/sign-in` with zero console errors — the actual create/display/invite flow needs a real session, which this sandbox can't reach against the live backend host (see [[flolyt_onboarding_build]] lesson 9). |
