...
const STEPS = ["Size and duration", "Who is excluded"];

/** The 2-pill wizard rail for /attribution/holdouts/new (AT11/AT12). */
export function NewHoldoutRail({ active }: { active: number }) {
  return (
      ...
      {STEPS.map((label, i) => {
        ...
        return (
          <div
            key={label}
            className={cn(
              "flex flex-1 items-center gap-2 rounded-panel border px-3 py-2.5",
