...
import { NEW_AGENT_STEPS } from "@/oldpages/agents/agent-builder/data";

/** The 6-pill wizard rail for /agent-builder/new (AB04-AB09). */
export function NewAgentRail({ active }: { active: number }) {
  return (
    <div className="flex flex-wrap gap-2">
      {NEW_AGENT_STEPS.map((label, i) => {
        const stepNum = i + 1;
        ...
        return (
          <div
            key={label}
            className={cn(
              "flex min-w-[120px] flex-1 shrink-0 items-center gap-2 rounded-panel border px-3 py-2.5",
