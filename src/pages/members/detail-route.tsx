...
/**
 * Same team page as /onboarding/team/:teamId (team-detail-route.tsx) — same hooks, same
 * members/invitations tables and modals, minus the wizard chrome. No WizardStepper, and no
 * "Back to teams" CTA at the bottom: the breadcrumb (Members / <team name>, via
 * usePageBreadcrumb) already covers going back, per this app's breadcrumb-only nav convention.
 */
export default function MembersDetailRoute() {
  ...
  usePageBreadcrumb(
    team
      ? [{ label: "Members", to: "/members" }, { label: team.name }]
    ...
    <div className="mx-auto max-w-4xl space-y-8">
      {isLoading || !team ? (
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-[17px] font-semibold text-ink">{team.name}</h1>
            <p className="mt-1 text-[11.5px] text-ink-3">{team.description || "No description yet"}</p>
          </div>
